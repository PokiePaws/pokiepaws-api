...
public class ClinicAssortmentService {
  ...
  private void decrementStock(String productName, int orderedAmount) {
    Optional<WarehouseStockItem> stockOpt =
        warehouseStockItemRepository.findByNameIgnoreCase(productName);
    if (stockOpt.isEmpty()) {
      log.warn(
          "No warehouse stock item found for name '{}', skipping stock decrement.", productName);
      return;
    }
