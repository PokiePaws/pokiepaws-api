...
public class WarehouseStockService {
    ...
    public List<WarehouseStockItemResponse> getLowStock(int threshold) {
        return repository.findAllByAmountLessThanEqual(threshold).stream()
                .map(this::toResponse)
                .toList();
    }
    ...
    public WarehouseStockItemResponse create(WarehouseStockItemRequest request) {
        ...
        if (repository.existsByNameIgnoreCaseAndWarehouse_Id(request.getName(), warehouse.getId())) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "Product with this name already exists in the warehouse");
        }
    ...
    private WarehouseStockItem findById(Long id) {
        return repository
                .findById(id)
                .orElseThrow(
                        () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Stock item not found"));
    }
